...
package com.touchdown.muchface.live.facedetectcamera.activity.ui;
...
public class FaceOverlayView extends View {
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (mFaces != null && mFaces.length > 0) {
      ...
      for (FaceResult face : mFaces) {
        ...
        if (mid.x != 0.0f && mid.y != 0.0f) {
          ...
          rectF.set(new RectF((mid.x - eyesDis * 1.2f) * scaleX, (mid.y - eyesDis * 0.65f) * scaleY,
              (mid.x + eyesDis * 1.2f) * scaleX, (mid.y + eyesDis * 1.75f) * scaleY));
          if (isFront) {
          ...
          canvas.drawText("ID " + face.getId(), rectF.left, rectF.bottom + mTextPaint.getTextSize(),
              mTextPaint);
          canvas.drawText("Confidence " + face.getConfidence(), rectF.left,
              rectF.bottom + mTextPaint.getTextSize() * 2, mTextPaint);
          canvas.drawText("EyesDistance " + face.eyesDistance(), rectF.left,
              rectF.bottom + mTextPaint.getTextSize() * 3, mTextPaint);
        }
      }
      canvas.restore();
    }
    ...
    canvas.drawText(
        ...
        mTextPaint.getTextSize(), mTextPaint.getTextSize(), mTextPaint);
  }
